...
class TrainDiffusionUnetHybridWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        # share rank-0's output_dir with all processes so checkpoints/logs land together
        ...
        # its step counter, e.g. 687) while global_step resets to 0 because no
        # checkpoint had been saved yet -> every log lands below the current
        # wandb step and is silently dropped.
        ...
        # wandb step pointer and global_step stay in sync. A fresh run (no
        # checkpoint loaded) must start a new wandb run at step 0.
        ...
        # NOTE: env_runner is instantiated lazily inside the rollout block below.
        # Eager construction fails for robocasa tasks because the task config's
        # env_kwargs no longer carries `env_name` (it is injected per-task by the
        # standalone eval_robocasa.py). Mirrors the transformer workspace.
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                for batch_idx in tqdm.tqdm(range(cfg.training.max_train_steps), desc=f"Training epoch {self.epoch}", leave=False, mininterval=cfg.training.tqdm_interval_sec):
                    ...
                    # compute loss (route through forward so DDP syncs grads)
